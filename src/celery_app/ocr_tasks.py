...
def process_ocr_task(self, task_key: str) -> bool:
    ...
    """
    logger.info(f"开始处理OCR任务，任务键：{task_key}")  # 关键入口日志保留
    config = ApiConfig()
    db = None
    ...
    try:
        ...
        logger.info(f"获取到OCR任务数据（记录ID：{record_id}，待处理URL数：{len(urls)}）")  # 关键信息保留
        ...
        try:
            ...
            if not need_ocr:
                logger.info(f"OCR记录已处理成功（记录ID：{record_id}），清理任务键：{task_key}")  # 关键清理日志保留
            ...
            for url in urls:
                try:
                    logger.debug(f"开始处理URL：{url}，记录ID：{record_id}")  # 降级为debug（原info）
                    # 调用Dify API识别图片内容
                    ...
                    logger.debug(f"URL处理成功，URL：{url}，记录ID：{record_id}")  # 降级为debug（原info）
                except Exception as e:
                    all_url_success = False  # 任意URL失败则标记整体失败
                    logger.error(f"URL处理失败（记录ID：{record_id}）：URL={url}，错误详情：{str(e)}", exc_info=True)  # 关键错误保留
                    break  # 失败时终止遍历，不再处理后续URL

            # 更新数据库OCR结果
            logger.debug(f"准备更新数据库（记录ID：{record_id}）")  # 降级为debug（原info）
        ...
        logger.error(f"任务处理失败（任务键：{task_key}）：{str(e)}", exc_info=True)  # 关键异常日志保留
...
def setup_periodic_tasks(sender, **kwargs):
    ...
    try:
        config = CeleryConfig()
        logger.info(f"注册定时任务：扫描Redis OCR任务（间隔{config.CELERY_SCAN_TASKS_INTERVAL}秒）")  # 关键注册日志保留
...
def scan_redis_tasks():
    ...
    try:
        ...
        if tasks:
            logger.info(f"扫描到{len(tasks)}个待处理OCR任务（限制{batch_size}条）")  # 关键扫描结果保留
            for key in tasks:
