...
def process_ocr_task(self, task_key):
    ...
    try:
        ...
        logger.debug(f"构建图片URL: {image_url}")
        try:
            dify_client = get_dify_client()
            logger.debug(f"调用Dify服务: ")
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(sql, (f"{task_key}_{self.request.id}", json.dumps(answer, ensure_ascii=False), datetime.now(), record_id))
